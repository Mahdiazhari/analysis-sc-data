# Plot cell line dendrograms from hierarchical clustering with each measure of association.
...
file = ("results/cor/GSE81861-rho_p.Rdata")
...
load(file, verbose = T) ## coexpr
  ...
  p2rho = ggplot(hm, aes(x = idx, y = '1', fill = cell_type)) + 
    geom_tile() +
    scale_y_discrete(expand = c(0, 0)) +
    scale_fill_manual(values = pal1, name = 'cell type', guide = "none") +
    theme_sc + 
  ...
  p3rho = ggplot(hm, aes(x = idx, y = '1', fill = batch)) + 
    geom_tile() + 
    scale_y_discrete(expand = c(0, 0)) +
    scale_fill_manual(values = pal2, name = 'batch', guide = "none",
                      na.value = 'grey90') +
    theme_sc + 
    theme(axis.text = element_blank(),
          axis.line = element_blank(),
  ...
  # plot together
  prho = p1rho + p2rho + p3rho + plot_layout(ncol = 1, heights = c(8, 1, 1))
  prho
  # save
  ggsave(paste0("results/fig/dendrogram-rho.pdf"),
         prho, width = 4, height = 3, units = 'cm', useDingbats = F)
...
legend
ggsave("results/fig/dendrogram-legendrho.pdf", legend, width = 10, height = 10, 
