...
files = list.files("/Users/mbmedrano/Documents/BSE DSM/4 Master Thesis/Thesis Code/thesis/results/cor/rev", pattern = "*.Rdata", 
                   full.names = T)
for (file in files) {
  ...
  # add to results
  #results %<>% rbind(list(coefficient = coef, ARI = ARI, ARI_batch = ARI_batch, NMI = NMI, NMI_batch = NMI_batch))
  results %<>% rbind(list(coefficient = coef, ARI = ARI, NMI = NMI))
}

# Write
write.csv(results, "results/clustering/hclust_rev.csv", row.names = F)
